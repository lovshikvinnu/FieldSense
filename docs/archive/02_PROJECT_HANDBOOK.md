...
- **Status**: `PLANNED` / downstream consumer.
- **Boundary**: Optional local LLM module. Consumes structured deterministic results (`FieldIntelligenceResult`, `ZoneDetectionResult`, `RecommendationResult`) to generate natural language summaries. Cannot alter deterministic scores or invent new metrics.
